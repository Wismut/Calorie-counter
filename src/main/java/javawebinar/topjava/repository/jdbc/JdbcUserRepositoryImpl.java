...
//				.addValue("caloriesPerDay", user.getCaloriesPerDay()); FIXME
...
//		users.forEach(u -> u.setRoles(userRoles.get(u.getId()).stream().map(UserRole::getRole).collect(Collectors.toList()))); FIXME
		users.forEach(u -> u.setRoles(userRoles.get(u.getId()).stream().map(UserRole::getRole).collect(Collectors.toSet())));
		return users;
	}
	...
	private User setRoles(User u) {
		...
		u.setRoles(new HashSet<>(roles)); // FIXME
		return u;
	}
